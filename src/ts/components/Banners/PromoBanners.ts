...
const fetchBanner = async (): Promise<BannersData[]> => {
    ...
    const promo_banners: BannersData[] = data.promo_banners ?? [];
...
/* ---------------- render Banners ---------------- */
const renderbanners = (promo_banners: BannersData[]): string => {
    return promo_banners
        .map((b) => `
            <li class="promo_banner">
                <a href="${b.url}">
                       <img
...
/* ---------------- load Banners ---------------- */
export const loadPromoBanners = async (): Promise<void> => {
    const container = document.querySelector('#promo_banner_list');
    ...
    try {
        const promo_banners = await fetchBanner();
        container.innerHTML = renderbanners(promo_banners);
        initBannerImages(container);
    } catch (error) {
        console.error('Failed to loadPromoBanners:', error);
        container.innerHTML = 'Error loadPromoBanners.';
    }
};

export default { loadPromoBanners };
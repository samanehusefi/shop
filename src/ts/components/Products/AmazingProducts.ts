...
/* ---------------- render Skeleton---------------- */
const createAmazingSkeleton = (count = 10) =>
...
`
        ...
        .join('');
/* ---------------- render ---------------- */
const createAmazingProduct = (items: ProductData[]) =>
    items
        .filter(pro => pro.is_amazing)
        .slice(0, 10)
        .map((pro, index) => {
            return `
<swiper-slide class="h-full !bg-white ${
...
/>
            <img
                 src="${pro.image}"
                 class="product-image w-full h-full object-fill px-1 py-1 opacity-0 
...
export default { loadAmazingProduct };